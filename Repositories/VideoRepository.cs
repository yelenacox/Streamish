...
namespace Streamish.Repositories
{
    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                          SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated, v.UserProfileId,
                                up.Id AS UserId, up.Name, up.Email, up.DateCreated AS UserProfileDateCreated, up.ImageUrl AS UserProfileImageUrl
                            FROM Video v
                                JOIN UserProfile up ON v.UserProfileId = up.Id
                           WHERE v.Id = @Id";
        ...
        public List<Video> VideoByDate(string criterion)
        {
                using (var conn = Connection)
            {
                conn.Open();
